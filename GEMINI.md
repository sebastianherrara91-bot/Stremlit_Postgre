...
- `GestorSQL.py`: A data access layer handling the dynamic connection to PostgreSQL. It checks `os.name` to optionally vary driver strings, masks credentials loaded via `python-dotenv`, and uses `SQLAlchemy` mapping with Python dictionaries to securely inject parameters using `CAST(:param AS TYPE)` standard logic. Recently optimized manually using `@st.cache_resource` for engine creation handling a Connection Pool (`pool_size=15`, `max_overflow=20`) to serve concurrent queries smoothly. Also enforces `search_path=dbo,public`. (Note: Changing string object types to Pandas `category` type caused instability across `groupby` components with `observed` parameters, thus regular string types are maintained).
- `Querys/`: Directory housing `.sql` files. These queries have been migrated from SQL Server logic to PostgreSQL natively (e.g. usage of `date_trunc`, `interval`, native min/max boundary search avoiding intense nested loops on large tables like `dwh_ventas`). Heavily optimized to avoid correlated/conditonal subqueries inside CTEs (e.g., `Valid_Marca_Tipo`) replacing `SELECT LIMIT 1` loops by direct `LEFT JOIN` mapping via `COALESCE()` aggregation over system catalogs (`marca_subclase`, `marca`, `monitoreo`), bringing high efficiency to heavy parallel fetching.
