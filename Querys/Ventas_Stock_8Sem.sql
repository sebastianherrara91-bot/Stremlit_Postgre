...
SELECT
    syv.ini_cliente AS "Ini_Cliente", syv.c_l AS "C_L", syv.local AS "Local", 
    syv.ean AS "EAN", syv.sku AS "SKU", syv.modelo AS "Modelo", syv.marca AS "Marca",
    syv.tipo_programa AS "Tipo_Programa", syv.fecha AS "Fecha",
    to_char(syv.fecha, 'YY') || '/' || to_char(syv.n_sem, 'FM00') || ' - ' || to_char(syv.fecha, 'MM/DD') as "Semanas",
    SUM(syv.cant_v) as "Cant_Venta", SUM(syv.cant_s) as "Cant_Stock",
    CASE WHEN SUM(syv.cant_v) = 0 THEN NULL ELSE ROUND(SUM(syv.cant_v * syv.pvp)/NULLIF(SUM(syv.cant_v),0),0) END as "PVP_Prom"
FROM (
    SELECT
        ST.ini_cliente, ST.num_local as c_l, T.local, ST.ean, EC.sku, EC.ref_modelo as modelo,
        VMT.vmt_marca as marca, VMT.vmt_tipo as tipo_programa,
    ...
    SELECT
        VT.ini_cliente, VT.num_local as c_l, T.local, VT.ean, EC.sku, EC.ref_modelo as modelo,
        VMT.vmt_marca as marca, VMT.vmt_tipo as tipo_programa,
